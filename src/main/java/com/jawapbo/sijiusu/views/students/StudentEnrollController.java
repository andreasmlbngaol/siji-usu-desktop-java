...
public class StudentEnrollController extends Controller {

    @FXML private VBox itemsContainer;

    @FXML
    private void initialize() throws JsonProcessingException {
        var student = Mapper.getInstance().readValue(
            ApiClient.get(Endpoint.GET_STUDENT_INFO.getPath()).body(),
            StudentResponse.class
        );


        var response = ApiClient.get(
            Endpoint.STUDENT_COURSES_SECTIONS.getPath()
        );
        ...
        try {
            ...
            sections
                .stream()
                .filter(section ->
                    student.coursesTaken().stream()
                        .noneMatch(taken -> taken.courseName().equals(section.courseName()))
                )
