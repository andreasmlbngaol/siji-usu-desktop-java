...
public class AdminDashboardAcademicMajorIdCoursesSectionCreateController extends Controller {
    ...
    private void onRegister() {
        ...
        if (name.isEmpty()) {
            StyledAlert.show("Error", "Name cannot be empty.");
            return;
        }
        ...
        var response = ApiClient.post(
            Endpoint.ADMIN_CREATE_SECTION_BY_COURSE_ID.getPath().formatted(DataManager.getCourseId()),
            body
        );

        try {
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                StyledAlert.show("Success", "Section created successfully.");
                switchScene(AppScene.ADMIN_DASHBOARD_ACADEMIC_MAJOR_ID_COURSE_SECTION);
            } else {
                StyledAlert.show("Error", "Failed to create section: " + response.body());
            }
        } catch (IOException e) {
            StyledAlert.show("Error", "Failed to switch scene: " + e.getMessage());
        }
    }
